...
def training_chunk(current, target, chunk_size):
    """Return the next training chunk without crossing the configured budget."""
    return max(0, min(chunk_size, target - current))


def atomic_torch_save(value, path):
    """Write a torch checkpoint via a same-directory temporary file."""
...
def simulate(
    ...
    while (steps and step < steps) or (episodes and episode < episodes):
        ...
        # v5 全量原子回流：本步算出的 atoms(t) 写进每个 env 的寄存器，
        # 供该 env 下一次 step()/reset() 内部构造 obs 时拼接使用（见
        # dreamer.py::Dreamer._policy() 和 envs/wrappers.py::AtomRegisterWrapper）
        if isinstance(action, list) and len(action) and isinstance(action[0], dict) \
                and "atoms" in action[0]:
            for env, a in zip(envs, action):
                env.set_register(a["atoms"])
        # step envs
            ...
            for i in indices:
                ...
                if not is_eval:
                    ...
                    if len(eval_scores) >= episodes and not eval_done:
                        ...
                        logger.write(step=logger.step)
                        # 供 dreamer.py::main() 的 eval_length 坍缩预警读取（write() 会清空
                        # logger._scalars，调用方拿不到刚写的值，所以额外存一份到 logger 自身）
                        logger.last_eval_return = score
                        logger.last_eval_length = length
                        eval_done = True
    if is_eval:
