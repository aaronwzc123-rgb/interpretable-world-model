...
class OnlineTrainer:
    def __init__(self, config, replay_buffer, logger, logdir, train_envs, eval_envs):
        self.demo_steps = int(getattr(config, "demo_steps", 0))
    ...
    def _save_checkpoint(self, agent):
        """Periodic checkpoint so a mid-run interruption doesn't lose all progress."""
        items_to_save = {
    ...
    def begin(self, agent):
        ...
        while step < self.steps:
            ...
            # Model 2: demo 预热期用求解器动作覆盖 (注入成功轨迹)
            # trans["demo_action"] 是 (B, A)（parallel.py 的 lift_dim 只对 ndim==1
            # 的标量字段加时间维，(B, A) 向量字段不受影响），不需要再 [:, 0] 去掉时间维。
            if "demo_action" in trans.keys() and step < self.demo_steps:
                act = trans["demo_action"].to(act.device).to(act.dtype)
