...
class Dreamer(nn.Module):
    def __init__(self, config, obs_space, act_space):
        ...
        # Anneal the entropy bonus down over training: once the policy converges,
        # advantage naturally shrinks (returns become uniform across the imagined
        # batch), and a fixed entropy coefficient then dominates the shrunk
        # advantage term and drags a converged policy back toward high entropy.
        self._act_entropy_min_frac = float(getattr(config, "act_entropy_min_frac", 0.05))
        self._act_entropy_decay_updates = float(getattr(config, "act_entropy_decay_updates", 8000))
        ...
        if getattr(config, "use_ndnf", False):
            self.rssm = ndnf_rssm.NDNFRSSM(
                config.rssm,
                self.embed_size,
                self.act_dim,
            )
            ...
            for name, module in mods.items():
                if isinstance(module, nn.Parameter):
                    ...
                    for param_name, param in module.named_parameters():
                        named[f"{name}.{param_name}"] = param
            return named

        # Split into 3 optimizer groups: world model (everything but actor/value),
        # actor, critic — separate LRs (aligned to the working reference config),
        # same LaProp/betas/eps/AGC for all three so this only tests LR-per-module.
        actor_modules = {"actor": self.actor}
        value_modules = {"value": self.value}
        wm_modules = {k: v for k, v in modules.items() if k not in ("actor", "value")}
        self._wm_params = _collect_params(wm_modules)
        self._actor_params = _collect_params(actor_modules)
        self._value_params = _collect_params(value_modules)
        self._named_params = OrderedDict()
        self._named_params.update(self._wm_params)
        self._named_params.update(self._actor_params)
        self._named_params.update(self._value_params)
        ...
        self._agc = _agc
        self._model_optimizer = LaProp(
            self._wm_params.values(),
            lr=config.model_lr,
            betas=(config.beta1, config.beta2),
            eps=config.eps,
        )
        self._actor_optimizer = LaProp(
            self._actor_params.values(),
            lr=config.actor_lr,
            betas=(config.beta1, config.beta2),
            eps=config.eps,
        )
        self._value_optimizer = LaProp(
            self._value_params.values(),
            lr=config.value_lr,
            betas=(config.beta1, config.beta2),
            eps=config.eps,
        )
        ...
        self._model_scheduler = LambdaLR(self._model_optimizer, lr_lambda=lr_lambda)
        self._actor_scheduler = LambdaLR(self._actor_optimizer, lr_lambda=lr_lambda)
        self._value_scheduler = LambdaLR(self._value_optimizer, lr_lambda=lr_lambda)
    ...
    def _current_act_entropy(self):
        frac = min(1.0, self._update_count / self._act_entropy_decay_updates)
        return self.act_entropy * (1.0 - frac + self._act_entropy_min_frac * frac)
    ...
    def update(self, replay_buffer):
        ...
        # unscale grads in params (must unscale each optimizer before its .step())
        self._scaler.unscale_(self._model_optimizer)
        self._scaler.unscale_(self._actor_optimizer)
        self._scaler.unscale_(self._value_optimizer)
        ...
        self._agc(self._named_params.values())  # clipping (per-tensor AGC, group-agnostic)
        self._scaler.step(self._model_optimizer)  # update params
        self._scaler.step(self._actor_optimizer)
        self._scaler.step(self._value_optimizer)
        self._scaler.update()  # adjust scale (once, shared across the 3 optimizers)
        self._model_scheduler.step()  # increment schedulers
        self._actor_scheduler.step()
        self._value_scheduler.step()
        self._model_optimizer.zero_grad(set_to_none=True)  # reset grads
        self._actor_optimizer.zero_grad(set_to_none=True)
        self._value_optimizer.zero_grad(set_to_none=True)
        mets["opt/lr_model"] = self._model_scheduler.get_lr()[0]
        mets["opt/lr_actor"] = self._actor_scheduler.get_lr()[0]
        mets["opt/lr_value"] = self._value_scheduler.get_lr()[0]
    ...
    def _cal_grad(self, data, initial):
        ...
        # === Model 2: N-DNF 谓词接地 + 离散化正则 + delta 退火 ===
        if isinstance(self.rssm, ndnf_rssm.NDNFRSSM):
            if "gt_preds" in data.keys():
                losses["ground"] = self.rssm.grounding_loss(
                    post_logit, to_f32(data["gt_preds"])
                )
            losses["ndnf_aux"] = self.rssm.aux_losses()
            metrics["ndnf_delta"] = self.rssm.step_delta()
        ...
        act_entropy_now = self._current_act_entropy()
        losses["policy"] = torch.mean(weight[:, :-1].detach() * -(logpi * adv.detach() + act_entropy_now * entropy))
        metrics["act_entropy_coef"] = act_entropy_now
