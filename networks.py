...
class RSSM(nn.Module):
    def __init__(
        ...
        self._embed = embed
        self._device = device
        self._atom_dim = int(atom_dim)   # v5 全量原子回流：img_step 的原子输入维度，默认0=原样不变
    ...
    def img_step(self, prev_state, prev_action, sample=True, atoms=None):
        ...
        # v5 全量原子回流：atoms 未提供时用零占位（atom_dim=0 时整段逻辑原样不变，
        # 向后兼容所有非 sym_recur_atoms 的任务/checkpoint）
        if self._atom_dim > 0:
            if atoms is None:
                atoms = torch.zeros(
                    prev_stoch.shape[0], self._atom_dim, device=prev_stoch.device
                )
            # (batch, stoch * discrete_num) -> (batch, stoch * discrete_num + action + atom_dim)
            x = torch.cat([prev_stoch, prev_action, atoms], -1)
        else:
            # (batch, stoch * discrete_num) -> (batch, stoch * discrete_num + action)
            x = torch.cat([prev_stoch, prev_action], -1)
        # (batch, stoch * discrete_num + action[+atom_dim], embed) -> (batch, hidden)
